...
import { Check, Gamepad2, Shield, Users, MessageSquare, Crown, Server, Youtube, MessageCircleMore, Play } from 'lucide-react'
...
function Home() {
  ...
  React.useEffect(() => {
    fetch(LINKS.STATUS_ENDPOINT).then(r => r.json()).then(data => { setStatus(data); }).catch(()=>{}).finally(()=>setLoading(false))
  }, [])
  ...
  const faqs = [
    ...
    { q: 'Jak získám WL?', a: 'Připoj se na Discord, přečti pravidla a otevři WL ticket. Formát a pokyny najdeš v #how-to-wl.' },
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-zinc-950 via-black to-zinc-950 text-white">
      ...
      <section id="faq" className="mx-auto max-w-3xl px-4 py-16">
        ...
        <Accordion className="p-2">
          {[
            ...
            <AccordionItem key={i} value={`item-${i}`}><AccordionTrigger className="text-left">{f.q}</AccordionTrigger><AccordionContent>{f.a}</AccordionContent></AccordionItem>
          ))}
        </Accordion>
      </section>
      ...
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 py-10 grid md:grid-cols-3 gap-8 text-sm">
          <div><div className="font-semibold">ElevateRP</div><p className="mt-2 text-white/70">Černo‑červený Elevate styl. Nahraď status API, jakmile budeš mít VPS/bota.</p></div>
          <div><div className="font-semibold mb-2">Odkazy</div><ul className="space-y-1"><li><a className="hover:underline" href="#rules">Pravidla</a></li><li><a className="hover:underline" href="#changelog">Changelog</a></li><li><a className="hover:underline" href="#support">Support</a></li></ul></div>
          <div><div className="font-semibold mb-2">Zůstaň v kontaktu</div><div className="flex gap-2"><a href={LINKS.DISCORD} target="_blank" rel="noreferrer"><Button variant="secondary" className="rounded-2xl">Discord</Button></a><a href={LINKS.YT} target="_blank" rel="noreferrer"><Button variant="secondary" className="rounded-2xl">YouTube</Button></a></div></div>
        </div>
